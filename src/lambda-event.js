 ...
 */
LambdaEvent.extractResourceFromEvent = (event, schema) => {
    if (_.isNil(event.body)) {
        throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.BODY_IS_NULL);
    }
    ...
    let resource = LambdaEvent.parseJSON(event.body, HttpError.statusCodes.BAD_REQUEST);
    LambdaEvent.validateJSON(resource, schema);
    return resource;
};
 ...
 */
LambdaEvent.parseJSON = (string, errorStatusCode) => {
    try {
        return JSON.parse(string);
    } catch (error) {
        throw new HttpError(errorStatusCode, LambdaEvent.Errors.JSON_IS_UNPARSABLE + error.message);
    }
};
 ...
 */
LambdaEvent.validateJSON = (object, schema) => {
    ...
    let validationError = new HttpError(
        ...
        LambdaEvent.Errors.JSON_IS_INVALID + JSON.stringify(schema)
    );
 ...
 * @returns {Date|*}
 */
LambdaEvent.deserializeDate = (value) => {
 ...
 */
LambdaEvent.mangleEntityDates = (entity) => {
    for (let key in entity) {
        if (entity.hasOwnProperty(key)) {
            let value = entity[ key ];

            let deserializedDate = LambdaEvent.deserializeDate(value);
...
/**
 * Receives a date and a modified guard string that express check. Verifies wether last modify date comes after control
 * string
 * @param lastModified Date to be checked
 * @param ifModifiedSinceStr Desired last modify string date
 * @returns {boolean}
 */
LambdaEvent.wasModifiedSince = (lastModified, ifModifiedSinceStr) => {
...
LambdaEvent.getMethodFromEvent = (event) => {
 ...
 */
LambdaEvent.getResourceUrlFromEvent = (event) => {
 ...
 */
LambdaEvent.extractPrincipalFromEvent = (event, principalSchema) => {
    ...
    if (!_.isString(principalId)) {
        throw new HttpError(HttpError.statusCodes.UNAUTHORIZED, LambdaEvent.Errors.PRINCIPAL_NOT_STRING);
    }
    let principal = LambdaEvent.parseJSON(
        principalId, HttpError.statusCodes.UNAUTHORIZED,
        LambdaEvent.Errors.PRINCIPAL_INVALID);
    LambdaEvent.validateJSON(
        principal,
        principalSchema,
 ...
 */
LambdaEvent.extractUUIDFromEvent = (event) => {
    if (event.pathParameters === null || event.pathParameters.uuid === undefined) {
        throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.UUID_MISSING);
    }
    let uuid = event.pathParameters.uuid;
    if (!_.isString(uuid)) {
        throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.UUID_NOT_STRING);
    }
    if (!uuid.toLowerCase().match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/)) {
        throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.UUID_NOT_MATCH);
    }
 ...
 * @returns {*}
 */
LambdaEvent.extractLocaleFromEvent = (event) => {
    ...
    if (_.isPlainObject(event.queryStringParameters) && _.isString(event.queryStringParameters.locale)) {
        ...
        if (qLocales === null) {
            throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.LOCALE_FORMAT);
        }
        ...
        if (hLocales === null) {
            throw new HttpError(HttpError.statusCodes.BAD_REQUEST, LambdaEvent.Errors.ACCEPT_HEADER_FORMAT);
        }
 ...
 */
LambdaEvent.resolveResourceUrl = (event, uuid) => {
    return LambdaEvent.getResourceUrlFromEvent(event) + '/' + uuid;
};
 ...
 * @returns {Object}
 */
LambdaEvent.buildSuccessResponseEvent = (event, result, statusCode = 200, headers) => {
    return LambdaEvent.buildResponseEvent(event, result, statusCode, headers);
};

LambdaEvent.buildResponseEvent = (inputEvent, body, statusCode, headers) => {
    let response = {
